...
st.markdown("""
    ...
    /* Style for the Master Vault feature */
    .vault-container { background-color: #1e2632; padding: 15px; border-radius: 10px; border-left: 5px solid #FFD700; margin-bottom: 20px; }
    </style>
...
# --- 5. DATA & SIDEBAR (MASTER VAULT FEATURE) ---
...
with st.sidebar:
    ...
    if not all_recs.empty:
        # PURI FAMILY KA TOTAL POOL (Saray accounts ka mila kar)
        grand_total = all_recs['Amount'].sum()
        st.markdown(f"""
            <div class="vault-container">
                <small style="color: #bbb;">Total Combined Pool</small><br>
                <span style="font-size: 22px; font-weight: bold; color: #FFD700;">PKR {grand_total:,.0f}</span>
            </div>
        """, unsafe_allow_html=True)
        
        # Breakdown of individual users
        st.markdown("#### 👥 Accounts Summary")
        summary = all_recs.groupby('Name')['Amount'].sum().reset_index()
        summary.columns = ['Name', 'Net Balance']
...
# --- 6. TRANSACTION CONFIRMATION (With Back-hand Individual Logic) ---
if st.session_state['confirm_mode']:
    preview = st.session_state['temp_data']
    st.warning("⚠️ **VERIFY DETAILS**")
    
    # Logic: Har bande ka balance uski entries ke mutabiq chalega
    ...
    updated_person_bal = old_person_bal + preview['Amount']
    ...
    <div class="confirm-card">
        ...
        <hr>
        <b>New Account Total:</b> PKR {updated_person_bal:,.0f}
    </div>
    """, unsafe_allow_html=True)
    
    cy, cn = st.columns(2)
    if cy.button("✅ Confirm & Save"):
        preview['Balance'] = updated_person_bal
