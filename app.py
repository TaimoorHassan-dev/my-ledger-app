...
if not all_recs.empty:
    # Errors='coerce' se Date ka masla khatam ho jayega
...
# --- TAB 1: MY LEDGER (Original Design) ---
with tab_personal:
                ...
                if st.form_submit_button("Preview"):
                    if n_in and a_in > 0:
                        st.session_state['temp_data'] = {
                            "Owner": st.session_state['username'], "Name": n_in, "Amount": a_in if type_in == "Received (+)" else -a_in,
                            "Currency": "PKR", "Type": type_in, "Date": d_in.strftime("%Y-%m-%d"), "Time": t_in.strftime("%H:%M:%S"), "Reason": r_in, "Balance": 0.0
                        }
...
with tab_insights:
    ...
    view = st.radio("View Basis:", ["Monthly", "Seasonal", "Annual"], horizontal=True)
    if not all_recs.empty:
        df_c = all_recs.copy()
        df_c['Month'] = df_c['Date'].dt.strftime('%b')
        df_c['Year'] = df_c['Date'].dt.year
        def get_s(m): return "Winter" if m in [12,1,2] else "Spring" if m in [3,4,5] else "Summer" if m in [6,7,8] else "Autumn"
        df_c['Season'] = df_c['Date'].dt.month.apply(get_s)
        
        g_col = "Month" if view == "Monthly" else "Year" if view == "Annual" else "Season"
        user_sums = df_c[df_c['Amount'] > 0].groupby(['Owner', g_col])['Amount'].sum().reset_index()
        fig = px.bar(user_sums, x=g_col, y="Amount", color="Owner", barmode="group")
