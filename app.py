...
# --- 2. NEW: CONFIRMATION VALIDATION LOGIC ---
if st.session_state['confirm_mode']:
    data = st.session_state['temp_data']
    st.warning("⚠️ **PLEASE CONFIRM YOUR TRANSACTION DETAILS**")
    ...
    <div class="confirm-box">
        ...
        <b>Purpose:</b> {data['Reason']}<br>
        <b>Date:</b> {data['Date']}
    </div>
    """, unsafe_allow_html=True)
    
    col_c1, col_c2 = st.columns(2)
    if col_c1.button("✅ Yes, Save it"):
        # Real Saving Process
    ...
    if col_c2.button("❌ No, Change it"):
        st.session_state.update({'confirm_mode': False, 'temp_data': None})
        st.rerun()
    st.stop() # Form hide kar deta hai jab tak confirm na ho

# --- 3. INPUT FORM ---
with st.expander("➕ Add New Transaction", expanded=True):
    with st.form("entry_form", clear_on_submit=True):
        col1, col2 = st.columns(2)
        name = col1.text_input("Person/Company Name")
        ...
        submit = st.form_submit_button("Preview Transaction")
        
        if submit:
            if name and amt > 0 and reason:
...
if not my_recs.empty:
    ...
    m1.metric("Received", f"{total_in:,.0f}")
    m2.metric("Sent", f"{total_out:,.0f}")
    m3.metric("Net Balance", f"{(total_in - total_out):,.0f}")
